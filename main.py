...
async def cmd_settings(message: Message):
    ...
    if not grupos:
        await message.answer("Nenhum grupo encontrado. Adiciona o bot aos teus grupos e dá admin.")
        return

    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text=nome, callback_data=f"select_group:{chat_id}")]
            for chat_id, nome in grupos.items()
            if isinstance(chat_id, int) and chat_id < 0
        ]
    )

    await message.answer("👉 Selecione o grupo que deseja configurar:", reply_markup=keyboard)
...
async def bot_added(event):
    if event.chat.type in ["group", "supergroup"]:
        ...
        if status in ["member", "administrator"]:
            grupos[event.chat.id] = event.chat.title
...
async def select_group(callback: CallbackQuery):
    ...
    grupo_selecionado[callback.from_user.id] = chat_id

    nome_grupo = grupos.get(chat_id, str(chat_id))
...
async def cb_settings(callback: CallbackQuery):
    ...
    if not grupos:
        await safe_edit(callback.message, "Nenhum grupo encontrado.")
        await safe_answer(callback)
        return
    ...
    await safe_edit(
        callback.message,
        "👉 Selecione o grupo que deseja configurar:",
...
async def cb_cfg_welcome(callback: CallbackQuery):
    ...
    modo = "Sempre enviar" if data["mode"] == "always" else "Enviar a mensagem de boas-vindas apenas na primeira entrada do usuário no grupo"
    apagar = "Ativado ✅" if data["delete_last"] else "Desativado ❌"

    submenu_welcome = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text="❌ Desativar", callback_data="welcome_disable"),
                InlineKeyboardButton(text="✅ Ativar", callback_data="welcome_enable"),
            ],
            [InlineKeyboardButton(text="✍🏻 Personalizar mensagem", callback_data="welcome_open_editor")],
            [
                InlineKeyboardButton(text="🔔 Sempre enviar", callback_data="welcome_mode_always"),
                InlineKeyboardButton(text="1️⃣ Enviar 1° entrada", callback_data="welcome_mode_first"),
            ],
            [InlineKeyboardButton(text=f"♻️ Deletar última mensagem {apagar}", callback_data="welcome_toggle_delete")],
            [InlineKeyboardButton(text="⬅️ Voltar", callback_data="settings")]
        ]
    )
    ...
    await safe_edit(
        callback.message,
        "💬 Mensagem de boas-vindas\n"
        "Nesse menu, você pode definir uma\n"
        "mensagem de boas-vindas que será\n"
        "enviada quando alguém entrar no\n"
        "grupo.\n\n"
        f"Status: {status}\n"
        f"Modo: {modo}",
        reply_markup=submenu_welcome
    )
    await safe_answer(callback)
...
async def capturar_configuracoes(message: Message):
    chat_id = get_selected_group(message.from_user.id)
    if not chat_id:
        return

    if chat_id in night_waiting_text_start:
        cur.execute("UPDATE night_config SET text_start=? WHERE chat_id=?", (message.text or "", chat_id))
        db.commit()
        night_waiting_text_start.discard(chat_id)
        await message.answer("✅ Texto de início salvo.")
        await message.answer(night_status_text(chat_id), reply_markup=menu_night_main(chat_id))
        return

    if chat_id in night_waiting_text_end:
        cur.execute("UPDATE night_config SET text_end=? WHERE chat_id=?", (message.text or "", chat_id))
        db.commit()
        night_waiting_text_end.discard(chat_id)
        await message.answer("✅ Texto de fim salvo.")
        await message.answer(night_status_text(chat_id), reply_markup=menu_night_main(chat_id))
        return
    ...
    if chat_id in night_waiting_media_start:
        if message.photo:
            ...
            await message.answer("Envie foto, vídeo ou documento.")
            return
        ...
        await message.answer("✅ Mídia de início salva.")
        await message.answer(night_status_text(chat_id), reply_markup=menu_night_main(chat_id))
        return
    ...
    if chat_id in night_waiting_media_end:
        ...
        await message.answer("✅ Mídia de fim salva.")
        await message.answer(night_status_text(chat_id), reply_markup=menu_night_main(chat_id))
        return
    ...
    while True:
        try:
            ...
            for row in chats:
                ...
                now = datetime.utcnow() + timedelta(hours=data["timezone_offset"])
                hour = now.hour
                key = f"{chat_id}:{now.strftime('%Y-%m-%d')}:{hour}"

                if hour == data["start_hour"] and night_last_event.get(chat_id) != key:
                    night_last_event[chat_id] = key
                    await send_night_message(chat_id, "start")

                if hour == data["end_hour"] and night_last_event.get(chat_id) != key:
                    night_last_event[chat_id] = key
                    await send_night_message(chat_id, "end")
...
async def main():
    ...
    await bot.set_my_commands([
        ...
        BotCommand(command="settings", description="Configurar grupos"),
